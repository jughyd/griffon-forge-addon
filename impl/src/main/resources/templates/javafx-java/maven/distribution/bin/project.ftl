...
##############################################################################
##
##  ${projectname} start up script for UN*X
##
##############################################################################
...
APP_NAME="griffon-sample-javax-java"
...
for jar_file in `ls $LIBDIR`; do
    CLASSPATH=$CLASSPATH:$LIBDIR/$jar_file
done
...
exec "$JAVACMD" "${JVM_OPTS}" -classpath "$CLASSPATH" org.buddha.Launcher "$@"
